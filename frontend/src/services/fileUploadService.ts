...
const CLOUDINARY_UPLOAD_PRESET = process.env.REACT_APP_CLOUDINARY_UPLOAD_PRESET || 'blog_images';
...
export const uploadImageFile = async (file: File): Promise<string> => {
  try {
    if (CLOUDINARY_CLOUD_NAME === 'YOUR_CLOUD_NAME') {
      throw new Error('Cloudinary Cloud Name not configured. Set REACT_APP_CLOUDINARY_CLOUD_NAME in .env.local');
    }

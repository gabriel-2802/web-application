...
const Comment: FC<CommentProps> = ({ comment, onCommentUpdate }) => {
  ...
  const fetchReplies = async (force = false) => {
    ...
    try {
      ...
      const response = await axios.get(COMMENT_ENDPOINTS.GET_REPLIES(comment.id.toString()), {
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this comment?')) {
      try {
        await axios.delete(COMMENT_ENDPOINTS.DELETE(comment.id.toString()), {
  ...
  return (
    <div className="comment">
      ...
      {isEditing ? (
        <div className="commentEditForm">
          ...
          <div className="commentEditActions">
            <button onClick={handleEdit} className="commentBtn save">
              Save
            </button>
            <button onClick={() => setIsEditing(false)} className="commentBtn cancel">
              Cancel
            </button>
          </div>
        </div>
      ...
      {(comment.hasReplies || replies.length > 0) && (
        ...
        >
          {loadingReplies ? 'Loading replies...' : showReplies ? 'Hide' : 'Show'} Replies ({replies.length})
        </button>
      )}

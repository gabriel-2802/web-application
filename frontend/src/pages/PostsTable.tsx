...
const PostsTable: FC = () => {
	...
	const handleDeleteConfirm = async () => {
		...
		try {
			setDeleting(true);
			await axios.delete(
				POST_ENDPOINTS.DELETE(deleteTargetId.toString()),
				{
					headers: { Authorization: `Bearer ${jwt}` },
				},
			);
			...
			setFilteredPosts(
				filteredPosts.filter((p) => p.id !== deleteTargetId),
			);
		...
		try {
			...
			if (modalMode === "add") {
				...
				const res = await axios.put(
					POST_ENDPOINTS.UPDATE(selectedPost.id.toString()),
					finalFormData,
					{
						headers: { Authorization: `Bearer ${jwt}` },
					},
		...
		try {
			if (collectionId) {
				...
				if (post?.collectionId) {
					await axios.put(
						COLLECTION_ENDPOINTS.REMOVE_POSTS(
							post.collectionId.toString(),
						),
						null,
						{
							params: { postIds: postId },
